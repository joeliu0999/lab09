...
template <typename T, typename F>
void reverse_traverse(LinkedList<T> const& q, F& dataFunction);
...
int main()
{
    ...
    LinkedList<char const*> pointerList;
    ...
    for (;;)
    {
        ...
        pointerList.dequeue();
        cout << endl << "\tReversed pointerList:";
        reverse_traverse(pointerList, printCString);
        cout << endl;
...
void reverse_traverse(LinkedList<T> const& q, F& dataFunction)
{
    ...
    for (auto itr = q.tail(); itr != q.end(); --itr)
    {
        dataFunction(*itr);
    }
    printCString("]");
